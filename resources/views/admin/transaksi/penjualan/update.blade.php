...
<form action="{{url('/admin/transaksi/penjualan/simpan_update')}}" class="form-penjualan" method="POST">
{{csrf_field()}}
                    ...
                    <div class="col-md-8">
                        <div class="form-inline" style="margin-top:10px;">
                            <div class="form-group">
                                ...
                                <input type="text" id="jumlah_bayar" name="jumlah_bayar" value="{{$penjualan->total_harga}}" readonly class="form-control" >
                                
                            </div>
                            <div class="form-group">
                                &nbsp;&nbsp;&nbsp;&nbsp;<label for="bayar" class="label-control">Bayar</label>
                                <input type="text" id="bayar" name="bayar" value="{{$penjualan->bayar}}" required class="form-control required bayar">
                            </div>
                            <div class="form-group">
                                &nbsp;&nbsp;&nbsp;&nbsp;<label for="kembali" class="label-control">Kembali</label>                               
                                <input type="text" id="kembali" name="kembali" value="{{$penjualan->kembali}}" readonly class="form-control kembali" >
                            </div>
                        </div>
                    </div>
                    <div class="col-md-4">
        ...
        <div class="row">
                    ...
                    <div class="panel-body">
                        <table class="table">
                            <tr>
                                <th>No Nota</th>
                                <td>:</td>
                                <td><input type="text" name="nota" class="form-control" value="{{$penjualan->nota}}"></td>
                            </tr>
                            <tr>
                                <th>Pelanggan</th>
                                <td>:</td>
                                <td>
                                    <select name="pelanggan" class="form-control select2Search">
                                        ...
                                        @foreach($data_pelanggan as $pelanggan)
                                            ...
                                            <option value="{{$pelanggan->id}}" {{$penjualan->kode_pelanggan == $pelanggan->id ? 'selected' : ''}}>{{$pelanggan->no_plat}}</option>
                                        @endforeach
                                    </select>
                                </td>
                            </tr>
                            <tr>
                                ...
                                <td>:</td>
                                <td>
                                    <select name="member" class="form-control select2Search">
                            ...
                            <tr>
                                <th>Tanggal Jual</th>
                                <td>:</td>
                                <td><input type="text" name="tanggal_jual" id="tanggal_jual" class="form-control" value="{{date('d-m-Y', strtotime($penjualan->tanggal_jual))}}"></td>
                            </tr>  
                            <tr>
                                <th>Teknisi</th>
                                <td>:</td>
                                <td>
                                    <select name="montir"  class="form-control select2Search">
                                        ...
                                        @foreach($data_montir as $montir)
                                            <option value="{{$montir->id}}" {{$penjualan->id_teknisi == $montir->id ? 'selected' : ''}}>{{$montir->nama_lengkap}}</option>
                                        @endforeach
                                    </select>
                                </td>
                            </tr>                          
                            <tr>
                                ...
                                <td><textarea name="keterangan" class="form-control" rows="4">{{$penjualan->keterangan}}</textarea></td>
                            </tr>
                        </table>
                    </div>
                </div>
            ...
            <div class="col-md-8">
                ...
                <div class="panel panel-primary">
                    <div class="panel-heading">
                        <h3 class="panel-title">Detile Penjualan Jasa</h3>
                    </div>
                    <div class="panel-body">                        
                        <div class="row">
                            <div class="col-md-12">
                                <div class="pull-right">
                                    <a class="btn btn-success btn-sm btn-tambah-service">Tambah Jasa</a>
                                </div>
                            </div>
                        </div>
                        <table class="table" >
                            <thead>
                                <tr>
                                    <th width="40%" style="text-align:center;">Service</th>
                                    <th style="text-align:center;">Harga</th>
                                    <th width="10%" style="text-align:center;">disc (%)</th>
                                    <th style="text-align:center;">Total Harga</th>
                                </tr>
                            </thead>
                            <tbody class="form_dynamic_service">
                                <?php $count_service = 0;?>
                                @foreach($penjualan->service as $s)
                                    <tr id="detile_service_{{$count_service}}">
                                        <td>
                                            <select name="detile_service[{{$count_service}}][id_service]" id="{{$count_service}}" class="form-control select_service select2Search">
                                                <option value="" selected disabled>Pilih Service</option>
                                                @forelse ($data_service as $service)
                                                    <option value="{{$service->id}}" {{$s->id_service == $service->id ? 'selected' : ''}}>{{$service->kode." | ".$service->nama}}</option>
                                                @empty
                                                    
                                                @endforelse
                                            </select>
                                        </td>
                                        <td><input name="detile_service[{{$count_service}}][harga]" id="{{$count_service}}" type="text" class="form-control text-center service_harga_{{$count_service}}" value="{{$s->harga}}" readonly></td>
                                        <td><input type="number" name="detile_service[{{$count_service}}][discount]" id="{{$count_service}}" value="{{$s->discount}}" class="form-control text-center service_discount_{{$count_service}}"></td>
                                        <td><input type="text" name="detile_service[{{$count_service}}][jumlah_harga]" id="{{$count_service}}" value="{{$s->subtotal}}" class="form-control detile_service_jumlah_harga_{{$count_service}} detile_service_jumlah_harga_" readonly></td>
                ...
                <div class="panel panel-primary">
                    <div class="panel-heading">
                        <h3 class="panel-title">Detile Penjualan</h3>
                    </div>
                    <div class="panel-body">
                        <div class="row">
                            <div class="col-md-12">
                        ...
                        <table class="table" >
                            ...
                            <tbody class="form-dynamic ">
                                ...
                                @if($penjualan->detile != null)
                                    @foreach($penjualan->detile as $d)
                                        <tr id="detile_penjualan_{{$count_detile}}">
                                            <td>
                                                <select name="detile_penjualan[{{$count_detile}}][barang]" id="{{$count_detile}}" class="form-control select_detile_barang select2Search">
                                                    <option value="" selected disabled>Pilih Barang</option>
                                                    @forelse ($data_barang as $barang)
                                                        <option value="{{$barang->id}}" {{$d->kode_sparepart == $barang->kode ? 'selected' : ''}}>{{$barang->kode_tipe_motor." | ".$barang->nama_barang}}</option>
                                                    @empty
                                                    @endforelse
                                                </select>
                                            </td>
                                            <td><input name="detile_penjualan[{{$count_detile}}][harga_jual]" value="{{$d->harga_jual}}" id="{{$count_detile}}" type="text" class="form-control text-center detile_harga_jual_{{$count_detile}}" readonly></td>
                                            <td><input type="number" name="detile_penjualan[{{$count_detile}}][jumlah]" value="{{$d->jumlah_sp}}" id="{{$count_detile}}" class="form-control text-center detile_jumlah row_jumlah_{{$count_detile}}"></td>
                                            <td><input type="text" name="detile_penjualan[{{$count_detile}}][disc]" id="{{$count_detile}}" value="{{$d->discount}}"  class="form-control detile_disc_{{$count_detile}}"></td>
                                            <td><input type="text" name="detile_penjualan[{{$count_detile}}][jumlah_harga]" id="{{$count_detile}}" value="{{$d->sub_total}}" class="form-control detile_jumlah_harga_{{$count_detile}} detile_jumlah_harga" readonly></td>
    ...
    <script>
        ...
        @if($penjualan->detile != null)
            <?php $i_detile = 0; ?>
            @foreach($penjualan->detile as $d)
                addJumlahEventListener({{$i_detile}});
                addEventListener({{$i_detile}});
                <?php $i_detile++ ?>
            @endforeach
            count_row = {{$penjualan->detile()->count()}};
        @endif
        var count_row_service = 0;
        @if($penjualan->service != null)
            
            count_row_service = {{$penjualan->service()->count()}};
        @endif
